...
## 개정 — 받은 파일 이름이 `%EC%96%91....hwp` 로 저장되던 버그

**증상**: 공유 링크(`/s/{토큰}`)에서 첨부를 받으면 파일명이 퍼센트 인코딩 그대로
저장됐습니다. 화면 목록에는 한글 이름이 멀쩡히 보여서 더 늦게 발견됐습니다.

### 1. 원인 — 우리 인코딩이 아니라 **서버가 보내는 헤더**였다

실제 응답을 떠서 확인했습니다 (service_role 로 서명 → 그 주소를 fetch → 헤더 확인):

```
content-disposition: attachment;
  filename=%EC%96%91%EC%8B%9D%20(%EC%B5%9C%EC%A2%85)%20%EA%B3%84%ED%9A%8D%EC%84%9C.hwp;
  filename*=UTF-8''%EC%96%91%EC%8B%9D%20(%EC%B5%9C%EC%A2%85)%20%EA%B3%84%ED%9A%8D%EC%84%9C.hwp
```

- `createSignedUrl({ download: 원본명 })` 에 **인코딩하지 않은 원본을 넘기는 것이 맞습니다.**
  supabase-js 가 `URLSearchParams` 로 한 번만 인코딩합니다 — 이중 인코딩이 아닙니다.
- 문제는 서버가 만든 헤더입니다. `filename*` 값에 괄호·공백이 **원문자 그대로** 들어가는데,
  RFC 5987 의 `attr-char` 에는 `(`·`)` 가 없습니다. 브라우저가 그 줄을 버리고 앞의
  `filename=` 을 쓰면, 그 값은 퍼센트 인코딩된 문자열이라 **그 글자 그대로** 저장됩니다.
- 즉 우리 쪽 코드로는 고칠 수 없는 자리입니다. 이름을 ASCII 로 바꾸면 헤더는 깨끗해지지만
  그건 한글 이름을 버리는 것이라 고침이 아닙니다.

### 2. 수정 — 헤더 의존을 끊는다 (fetch → blob → `a[download]`)

`supabase.js` 의 `downloadAsBlob(url, name)` 한 곳으로 모았습니다. 받아서 blob 으로 만들면
주소가 `blob:` 이 되어 **같은 출처**가 되고, 그때는 `<a download>` 속성이 이름을 확정합니다.
(다른 출처의 주소에서는 그 속성이 무시되기 때문에 지금까지 서버 헤더에 기대고 있었습니다.)

- 🔴 받다가 실패하면 원래 주소를 그냥 엽니다. 이름이 깨질지언정 파일은 손에 넣어야 합니다.
- `Ctrl/⌘/Shift + 클릭`(새 탭)은 가로채지 않습니다. `<a href>` 도 그대로 둡니다.
- 스토리지 CORS 는 `access-control-allow-origin: *` 이라 받는 쪽에서 fetch 가 막히지 않습니다(실측).

### 3. 본 계정 쪽도 같이 고쳤다 (재발 방지)

항목 모달의 내려받기도 **똑같은 결함**이 있었습니다 (`<a download>` + 서명 주소).
공유 쪽에서만 보고된 이유는 본인 파일은 이름을 알아보고 넘어갔기 때문으로 보입니다.
두 길이 서로 다른 방식을 쓰면 한쪽만 고치고 끝나므로 **한 함수로 합쳤습니다.**

### 4. 🔴 이미 만들어 둔 공유 링크는 소급되지 않는다

공유 링크는 만들 때 서명 주소를 `shares.files` 에 **굳혀 둡니다**(유효기간과 같은 수명).
이번 수정은 받는 쪽 코드가 하는 일이라 **이미 배포된 화면에서는 새 코드로 동작**하지만,
서비스워커 캐시에 옛 번들이 남아 있는 기기에서는 v6 로 갱신되기 전까지 옛 방식으로 받습니다.
링크 자체를 다시 만들 필요는 없습니다 — 화면에 안내 문구를 넣지 않은 이유입니다.

### 5. 검증

| 점검 | 결과 |
| --- | --- |
| `npm run check:share` | ALL PASS (99) — 받기 6건 추가 |
| `npm run check` · `check:files` · `check:mobile` | 105 · 377 · 69 전부 PASS |

받기 판정은 **실제 저장 이름 기준**입니다: `양식 (최종) 2026 계획서.hwp` (한글·공백·괄호)
로 공유 링크를 만들고, 눌렀을 때 ⑴ 서명 주소를 직접 fetch 하는지 ⑵ `blob:` 주소로
누르는지 ⑶ `a[download]` 가 원본명 그대로인지 ⑷ 이름에 퍼센트 인코딩이 새지 않는지,
그리고 ⑸ 받기가 실패하면 원래 주소로 열어 주는지까지 봅니다.
