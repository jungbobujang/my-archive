...
카드 구조와 톤은 그대로 두고, 처음 보는 사람이 이게 뭔지 알 수 있게만 보탰습니다.

- 소개 한 줄: "아이디어 · 대본 · 링크 · 할 일을 한곳에 모아 두는 나만의 아카이브"
  (기존 "아이디어, 대본, 이미지를 한곳에" 는 링크·할 일이 빠져 현재 앱과 맞지 않았습니다)
- 얇은 구분선 사이에 실제로 있는 기능 3가지를 한 줄로: ☀️ 오늘 할 것 / 🗂 카테고리·태그 / 🗺️ 마인드맵
- 카드 우상단에 테마 토글 — **다크 사용자가 로그인 전에는 테마를 바꿀 방법이 아예 없었습니다**
- 이메일 입력에 `autoFocus`
- 안내 문구에 "혼자 쓰는 아카이브라 가입 절차가 없습니다" 를 앞세워, 가입 버튼을 찾는 일이 없게 했습니다

### 아이콘

- `public/favicon.svg` 추가 — 크기와 무관하게 선명합니다. PNG(32px)는 폴백으로 남겼습니다
- SVG 와 PNG 는 **같은 글리프 좌표**를 씁니다. `scripts/generate-icons.mjs` 와 `favicon.svg`
  양쪽에 서로를 가리키는 주석을 달아, 한쪽만 고치는 일을 막았습니다
- 앱 아이콘은 기존대로 any 2종 + maskable 2종 + apple-touch-icon
- 서비스워커 셸 목록에 favicon.svg 를 넣으면서 캐시 `VERSION` 을 v1 → v2 로 올렸습니다

---

## 11. 유료화 대비 구조 (결제 연동 없음)

**결제는 붙이지 않았습니다.** 화면과 자리만 만들어 뒀습니다.

### /pricing

- 라우팅 라이브러리를 새로 넣지 않고 History API 로 처리했습니다 (화면이 둘뿐입니다).
  `vite preview` 의 SPA 폴백으로 주소창에 `/pricing` 을 직접 쳐도 열리는 것을 확인했습니다.
- 무료 / 프로 2단 비교. 기존 카드·배지·버튼 스타일을 그대로 씁니다 (새 컴포넌트 언어 없음)
- 상단에 "준비 중" 배지, 프로 버튼은 `disabled`
- ⚠️ 표에 적힌 한도(1,000개 · 100MB 등)는 **자리표시 숫자이고 어디에서도 강제되지 않습니다.**
  `Pricing.jsx` 맨 위 주석과 화면 하단 문구에 모두 적어 뒀습니다.

### 설정 모달

- ⋯ 메뉴의 테마 순환 버튼을 **설정 모달**로 바꿨습니다.
  같은 설정을 두 군데서 만지지 않도록 하나로 모았습니다(작업 8의 일관성 기준).
  로그인 화면에는 모달을 띄울 맥락이 없어 거기만 빠른 토글을 둡니다.
- 담긴 것: 화면 테마(시스템/라이트/다크 3택), **플랜: 무료 배지 + 요금제 보기**, 로그인 이메일
- 플랜 값은 하드코딩입니다. 구독 상태를 읽어 올 곳이 아직 없다는 주석을 코드에 남겼습니다.
